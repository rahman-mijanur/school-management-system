...
static void handleChangeUsernameUI(void) {
    ...
    if(strcmp(currentUser->username, newUsername) == 0) {
        showError("Try a different & unique name!");
        pauseAndClear();
        return;
    }
...
static void handleChangePasswordUI(void) {
    ...
    if(strcmp(newPassword, oldPassword) == 0) {
        showError("Try a different & unique password!");
        memset(oldPassword, 0, sizeof(oldPassword));
...
void showAccountSettingsUI(void) {
    int running = 1;
    ...
    while(running) {
        ...
        printf(RED "  0." RESET " Back to Teacher Dashboard\n");
        printf("  ────────────────────────────────────────\n");
